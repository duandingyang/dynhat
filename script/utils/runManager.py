...
class RunManager():
    ...
    def save(self, dir, split_count, heads, temporal_attention_layer_heads, nout, nhid):
        pd.DataFrame.from_dict(
            self.run_data,
            orient='columns'
        ).to_csv(f'{dir}/{split_count}s_{heads}h_{temporal_attention_layer_heads}ddy_{nout}out_{nhid}hid_data.csv')

        with open(f'{dir}/{split_count}s_{heads}h_{temporal_attention_layer_heads}ddy_{nout}out_{nhid}hid_data.json', 'w', encoding='utf-8') as f:
