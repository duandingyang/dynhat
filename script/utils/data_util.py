...
def load_vgrnn_dataset(dataset, split_count):
    assert dataset in ['enron10', 'enron10_delta_0', 'enron184', 'dblp', 'uci', 'uci_delta_0', 'movielen', 'movielen_delta', 'movielen_delta_0', 'dyngem']  # using vgrnn dataset
    ...
    new_pedges, new_nedges = mask_edges_prd_new_by_marlin(adj_time_list)  # list 新出现的节点进行正采样和负采样, 
...
def loader(dataset='enron10', split_count=10):
    ...
    if dataset in ['enron10', 'enron10_delta_0', 'enron184', 'dblp', 'uci', 'uci_delta_0','movielen', 'movielen_delta', 'movielen_delta_0', 'dyngem']:
        data = load_vgrnn_dataset(dataset, split_count)
